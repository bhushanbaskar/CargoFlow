...
export function Navbar({ onRequestAccess, onLaunchApp }: NavbarProps) {
  ...
  const { currentRole, currentProfile } = useCargoFlow();
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        scrolled
          ? 'bg-white/80 backdrop-blur-md border-b border-zinc-200/80 shadow-2xs py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-zinc-100/90 p-1.5 rounded-full border border-zinc-200/80 shadow-2xs">
          {[
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button
            ...
            className="p-2 text-zinc-700 hover:text-zinc-950 rounded-xl bg-zinc-100 border border-zinc-200"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-b border-zinc-200 px-6 py-6 space-y-4 animate-in slide-in-from-top-4 duration-200 shadow-xl">
          <nav className="flex flex-col gap-2">
            {[
              ...
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className="text-left px-4 py-3 rounded-2xl text-sm font-bold text-zinc-800 hover:bg-zinc-100 transition-colors"
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="pt-4 border-t border-zinc-100 flex flex-col gap-2.5">
            <Link
              ...
              className="w-full py-3.5 rounded-full bg-lime-300 text-zinc-950 font-black text-sm flex items-center justify-center gap-2 shadow-sm"
            >
            ...
            <Link
              href="/login"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full py-3 rounded-full bg-zinc-100 text-zinc-900 font-bold text-xs hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2"
            >
